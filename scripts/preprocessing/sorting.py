...
for i in df_annot.index:
    ...
    elif (df_annot.loc[i, 'Max side int'] == 1) & (counter_1 < 40):
        list_ids.append(pid)
        counter_1 += 1
    elif (df_annot.loc[i, 'Max side int'] == 0) & (counter_0 < 40):
        list_ids.append(pid)
        counter_0 += 1
    else:
        df_annot = df_annot.drop(labels=i)
...
print(df_annot)
df_annot.to_csv(os.path.join(meta_folder, 'clinical_data_USZ.csv'))
